...
This is a quick edit of nathancolgate's S3SwfUpload Gem that works with Heroku. All you need to do is use <code>heroku config:add</code> to add the following environmental variables into your Heroku app:

AMAZON_ACCESS_KEY_ID

AMAZON_SECRET_ACCESS_KEY

AMAZON_S3_SWF_UPLOAD_BUCKET

AMAZON_S3_SWF_MAX_FILE_SIZE

AMAZON_S3_SWF_UPLOAD_ACL

The max file size defaults to 10MB and the ACL defaults to private. If you are missing any other information, an error will be raised. If you want to use this for production without Heroku, be sure to set the environmental variable, RACK_ENV, to "production". Heroku has this by default so if you are using it, you won't need to worry about that.
